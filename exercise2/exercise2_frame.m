...
xlabel('x');
ylabel('y');
title(['Artificial distortion field: a=', int2str(a), ',b=', int2str(b), ',d=', int2str(d)]);
...
XD2 = XU2 + (XU2 - XD2); % distorted = undistorted + deformation
YD2 = YU2 + (YU2 - YD2);
...
NumCorresp = size(XD2,1) * size(YD2,2);
...
for y = 1:sy 
    for x = 1:sx       
     c = 1;
        for i = 0:d
            for j = 0:(d-i)
                XDist(y,x) = XDist(y,x) + Uvec(c) * x^i * y^j;
                YDist(y,x) = YDist(y,x) + Vvec(c) * x^i * y^j;
                c = c + 1;
            end
        end
    end
